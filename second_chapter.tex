...
Let's denote $\mathcal{D}_N = \bigcup_{k \in S_N} D_k$ the finite subset of $\mathscr{U}_L$ obtained by the scanning procedure.
We can reduce our consideration to regular solutions which orbits visit only islands from $\mathcal{D}_N$.
Denote $\mathcal{O}_N$ this set of orbits.
If Hypothesis~\ref{hypothesis:strips-mapping} is valid for the $\mathcal{D}_N$, we can conclude that conditions of Theorem~\ref{thm:coding} and there exists a homeomorphism between a {\it subset of orbits of regular solutions} $\mathcal{O}_N$ and set of bi-infinite sequences $\mathcal{S}_N$ of symbols from finite set $S_N$.
...
\begin{itemize}
	\item If linear operator $D \mathcal{P}_{\vb{p}} = (a_{mn})$, $\vb{p} \in V_{ij}$, satisfies {\it some conditions} on signs of $a_{mn}$, then $\mathcal{P}$ {\it maps h-strips to h-strips}.
	\item If for the linear operator $D \mathcal{P}_{\vb{p}} = (a_{mn})$ the estimation $|a_{11}| \ge \mu > 1$, $\exists \mu$, takes place for $\vb{p} \in V_{ij}$, then $\mathcal{P}$ {\it shrinks thickness of h-strips}.
\end{itemize}
...
\begin{itemize}
	\item If linear operator $D \mathcal{P}_{\vb{q}}^{-1} = (b_{mn})$, $\vb{q} \in H_{ij}$, satisfies {\it some conditions} on signs of $b_{mn}$, then $\mathcal{P}^{-1}$ {\it maps v-strips to v-strips}.
	\item If for the linear operator $D \mathcal{P}_{\vb{q}}^{-1} = (b_{mn})$ the estimation $|b_{22}| \ge \nu > 1$, $\exists \nu$, takes place for $\vb{q} \in H_{ij}$, then $\mathcal{P}^{-1}$ {\it shrinks thickness of v-strips}.
\end{itemize}
...
That allows to turn theorems on h- and v-strips mapping into a numerical algorithm of Hypothesis~\ref{hypothesis:strips-mapping} validation, see Algorithm~\ref{algorithm:hypotheses-validation}.

\begin{algorithm}[H]
\caption{Numerical Validation of Hypothesis~\ref{hypothesis:strips-mapping}}
...
\begin{algorithmic}
	...
	\STATE \textbf{Step (2)}. Signs validation.
	\STATE
	\begin{enumerate}
...
If the validation provided by the steps above is successful then one have a numerical evidence that Hypothesis~\ref{hypothesis:strips-mapping} is also valid.
