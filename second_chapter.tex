...
If the validation provided by the steps above is successful then one have a numerical evidence that Hypothesis~\ref{hypothesis:strips-mapping} is valid.
...
This completes the validation of Hypothesis~\ref{hypothesis:strips-mapping}.

\begin{figure}[h]
\centering
	\includegraphics[scale = 1]{pic/hypotheses for piecewise equation}
	\caption{
		Illustration to the validation of Hypothesis~\ref{hypothesis:strips-mapping} for equation \eqref{eq:stationary-piecewise} with parameters $(L_*, L_0) = (2, 1)$ for three-component island set $\mathcal{D}_3 = \{ D_{-1}, D_0, D_{+1} \}$.
	}
