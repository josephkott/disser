function eigenvalues = get_spectrum_nho(params, X, U, N)
...
G{1} = omega - (X .^ 2) +   f_potential(params, X) .* (U .^ 2);
G{2} = omega - (X .^ 2) + 3*f_potential(params, X) .* (U .^ 2);
...
potential = params(3) + params(4) * cos(params(2) * x);
end