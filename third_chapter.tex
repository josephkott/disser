...
\end{equation}
Localized solutions satisfy the condition
\begin{equation}
	\lim \limits_{x \to \pm \infty} u(x) = 0,
\label{eq:localization}
\end{equation}
which implies that the function $u(x)$ is real, see \cite{AlfimovKonotopSalerno}.
We also assume that the potential $U(x)$ is absent, i.e. $U(x) \equiv 0$, so the effects produced by the periodic modulation of the nonlinearity are not obscured by the linear-lattice potential.
...
\end{equation}
Result GPE equation takes form
\begin{equation}
	i \Psi_t + \Psi_{xx} + (\alpha + \cos 2x) |\Psi|^2 \Psi = 0,
\label{eq:gpe-cosine}
\end{equation}
where the period is scaled to be $L = \pi$.
...
For that purpose authors used variational approximation method which requires initial solution shape guess.
...
Numerical results allows us to conclude that the sets $\mathscr{U}_L^{\pm}$ are unbounded spirals with infinite number of rotations around the origin, similar to what we saw previously for equation \eqref{eq:stationary-piecewise}.
...
\centering
	...
	\caption{
		...
		On panel (d) seven connected components of $\mathscr{U}_L$ form an island set.
	}
...
From that picture we conclude that Hypothesis~\ref{hypothesis:island-set} holds since each connected component of $\mathscr{U}_{\pi}$ represent a curvilinear quadrangle with monotonic boundaries.
Each boundary satisfies the conditions from Definition~\ref{def:island-set}.
The connected component can be enumerated with symbols $\{D_k\}$, $k = \pm 1, \pm 2, \dots$ for the components along the $u$ axis, and $k = \pm 1\mathrm{i}, \pm 2\mathrm{i}, \dots$ for the components along the $u'$ axis.
...
It means that condition (2d) of Theorem~\ref{thm:h-strips-mapping} takes place.
...
\centering
	...
	\caption{
		...
		Estimations of $\mu_*$ and $\nu_*$ values show with histograms below each island.
		...
		Sets $H_{+2, k}$ and $V_{k, +2}$ are not depicted since they are too thin and barely visible at this scale, but all the computation have been also provided for them and the result satisfy the above mentioned theorems as well.
	}
...
Several periodic solutions are presented in Figure~\ref{fig:solutions-cosine}, $\pi$-periodic in panel (a), and $2 \pi$-periodic in panels (b) and (c).
...
For example, solution, shown in panel (e), represents a so-called {\it dipole soliton} (DS), which is essentially confined to a single period of pseudopotential $P(x)$.
...
If it's possible to find such parameters that the Theorem~\ref{thm:coding} is applicable, then for these parameters we can describe at least a huge subset of bounded solutions.
Using methods of numerical continuation one can continue such solutions to the area of parameters where coding is not possible.
That's allow to get a solutions structure even if the coding is not possible.
FCM is quite efficient and can be performed by a computer pretty fast.
That allows to visualize branches of solutions along with their linear-stability in a following way.
We fix parameters and perform the coding of solutions.
After that we vary one of the parameters using a numerical grid.
We perform numerical continuation of the obtained solutions and compute linear-stability spectrum in each point of the grid.
Then we can plot the obtained branches for different solution families and color the stability regions.

Illustration of the described procedure for Eq.~\eqref{eq:gpe-cosine} is presented in Figure~\ref{fig:branches-with-stability}.
...
One can see that solutions of code $\{ \dots, 0, \pm 1, 0, \dots \}$ (branch (I), corresponds to FS) and $\{ \dots, 0, \pm 1, 0 \mp 1, 0, \dots \}$ (branch (II)) are mostly stable and loose their stability near the point $\omega = 0$.
DS (branch (IV)) has an $\omega$-region of stability.
It exists for $\omega > \omega^*$.
...
Existence of such family is also predicted by variational approximation method, \cite{LebedevAlfimovMalomed}.
...
%... had not been previously reported for the considered setup of Eq.~\eqref{eq:gpe-cosine}.
%
%... were not previously considered in this setting.
%
%... first time published by us in [...].